...
# public repository yosishe/iqly-flow is written from, so the zip, the repository and the live link are the same bytes.
# Usage: tools/package.sh
set -e
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
STAGE="$ROOT/.iqly-stage"
rm -rf "$STAGE"; mkdir -p "$STAGE"
rsync -a --exclude '.DS_Store' --exclude 'MANIFEST.sha256' "$ROOT/submission/" "$STAGE/"
rsync -a --exclude '.DS_Store' --exclude '__pycache__' --exclude '*.pyc' "$ROOT/tools/" "$STAGE/tools/"
( cd "$STAGE" && find . -type f ! -name MANIFEST.sha256 ! -name .DS_Store | sed 's|^\./||' | LC_ALL=C sort | xargs shasum -a 256 > MANIFEST.sha256 )
...
( cd "$T" && shasum -a 256 -c MANIFEST.sha256 > check.txt; OK=$(grep -c ': OK$' check.txt); BAD=$(grep -vc ': OK$' check.txt || true); echo "fresh extraction: $OK files verified, $BAD failed ($(find . -type f ! -name check.txt | wc -l | tr -d ' ') files extracted)"; [ "$BAD" = "0" ] )
rm -rf "$T"
echo "manifest entries: $(wc -l < "$ROOT/submission/MANIFEST.sha256" | tr -d ' ')"
ls -l "$ROOT/iqly-submission.zip"
shasum -a 256 "$ROOT/iqly-submission.zip"
